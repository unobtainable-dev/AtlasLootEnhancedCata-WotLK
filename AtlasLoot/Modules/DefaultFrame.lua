...
local function updateLock()
    ...
    local prefix = "LockButton-"

    -- Redirect to your custom ElvUI textures if the skin is enabled
	if _G["ElvUI"] and _G["ElvUI"][1].private.addOnSkins and _G["ElvUI"][1].private.addOnSkins.AtlasLoot then
        path = "Interface\\AddOns\\AtlasLoot\\Images\\AtlasImages\\"
        prefix = "Elvui-LockButton-"
    end

    if(db.DefaultFrameLocked) then
        DefaultFrame.Frame.LockButton.NormalTexture:SetTexture(path..prefix.."Locked-Up")
        DefaultFrame.Frame.LockButton.PushedTexture:SetTexture(path..prefix.."Locked-Down")
    else
        DefaultFrame.Frame.LockButton.NormalTexture:SetTexture(path..prefix.."Unlocked-Up")
        DefaultFrame.Frame.LockButton.PushedTexture:SetTexture(path..prefix.."Unlocked-Down")
    end
end
...
local function onCompareFrameClick()
		if db.instance then
            -- FIX: Added existence check for CompareFrame
			if AtlasLoot.CompareFrame and AtlasLoot.CompareFrame:IsShown() then AtlasLoot.CompareFrame:Hide() end
	...
	function DefaultFrame:CreateDefaultFrame()
		...
		Frame.CompareFrame:SetWidth(0)
		Frame.CompareFrame:SetHeight(0)
